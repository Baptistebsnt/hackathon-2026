...
@Component({
        ...
        <div class="info-grid">
          <div class="info-block">
            ...
            <table class="info-table">
              <tr><td>Surface</td><td>{{ site.surfaceM2 | number }} m²</td></tr>
              <tr><td>Parking</td><td>{{ site.parkingSpots }} places</td></tr>
              <tr><td>Énergie</td><td>{{ site.energyConsumptionMwh | number }} MWh/an</td></tr>
              <tr><td>Employés</td><td>{{ site.employees }}</td></tr>
              <tr><td>Postes</td><td>{{ site.workstations }}</td></tr>
            </table>
          </div>
          <div class="info-block">
            <h3>Matériaux de construction</h3>
            <table class="info-table" *ngIf="site.materialBreakdown?.length">
              <thead><tr><th>Matériau</th><th>Quantité</th><th>CO₂</th><th>%</th></tr></thead>
              <tbody>
  ...
  `,
  styles: [`
    .detail-page { max-width: 900px; margin: 0 auto; padding: 2rem; font-family: 'Inter', sans-serif; }
    ...
    .kpi-card { background: white; border-radius: 10px; padding: 1rem; box-shadow: 0 1px 3px rgba(0,0,0,.06); text-align: center; }
    .kpi-card.highlight { background: #0f172a; color: white; }
    .kpi-card.construction .value { color: #d97706; }
    .kpi-card.operation .value { color: #2563eb; }
    .label { font-size: 0.7rem; color: #94a3b8; text-transform: uppercase; letter-spacing: .5px; }
    .kpi-card.highlight .label { color: #93c5fd; }
    .value { font-size: 1.2rem; font-weight: 700; color: #0f172a; margin-top: 4px; }
    .kpi-card.highlight .value { color: white; }
    .info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; }
    .info-block { background: white; border-radius: 12px; padding: 1.5rem; box-shadow: 0 1px 3px rgba(0,0,0,.06); }
    .info-block h3 { font-size: 0.9rem; font-weight: 600; margin-bottom: 1rem; color: #374151; }
    .info-table { width: 100%; border-collapse: collapse; font-size: 0.875rem; }
    .info-table th { font-size: 0.75rem; color: #64748b; padding: 6px 8px; text-align: left; border-bottom: 1px solid #f1f5f9; }
    .info-table td { padding: 8px; border-bottom: 1px solid #f8fafc; }
    .bar-wrap { display: inline-block; width: 60px; height: 6px; background: #f1f5f9; border-radius: 3px; vertical-align: middle; margin-right: 6px; }
    .bar { display: block; height: 100%; background: #6366f1; border-radius: 3px; }
