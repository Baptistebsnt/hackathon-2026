...
@Component({
  ...
  styles: [`
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Header */
    .header { background: #0f172a; color: white; padding: 0 2rem; }
    ...
    .btn-new { background: #22c55e; color: white; padding: 8px 16px; border-radius: 6px; text-decoration: none; font-size: 0.875rem; font-weight: 600; }

    /* Main */
    .main { max-width: 1200px; margin: 0 auto; padding: 2rem; }

    /* KPIs */
    .kpi-section { display: grid; grid-template-columns: repeat(4, 1fr); gap: 1rem; margin-bottom: 2rem; }
    .kpi-card { background: white; border-radius: 12px; padding: 1.5rem; box-shadow: 0 1px 3px rgba(0,0,0,.06); text-align: center; }
    .kpi-card.total { background: linear-gradient(135deg, #0f172a, #1e40af); color: white; }
    .kpi-icon { font-size: 1.5rem; margin-bottom: 0.5rem; }
    .kpi-value { font-size: 1.75rem; font-weight: 700; }
    .kpi-card:not(.total) .kpi-value { color: #0f172a; }
    .kpi-label { font-size: 0.75rem; margin-top: 4px; opacity: 0.7; }

    /* Charts */
    .charts-section { display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 2rem; }
    .chart-card { background: white; border-radius: 12px; padding: 1.5rem; box-shadow: 0 1px 3px rgba(0,0,0,.06); }
    .chart-card.wide { grid-column: 1 / -1; }
    .chart-card h3 { font-size: 0.9rem; font-weight: 600; color: #374151; margin-bottom: 1rem; }
    canvas { max-height: 260px; }

    /* Table */
    .sites-section h2 { font-size: 1.1rem; font-weight: 600; margin-bottom: 1rem; color: #0f172a; }
    .sites-table { width: 100%; background: white; border-radius: 12px; box-shadow: 0 1px 3px rgba(0,0,0,.06); border-collapse: collapse; overflow: hidden; }
    .sites-table th { padding: 12px 16px; background: #f1f5f9; font-size: 0.75rem; font-weight: 600; color: #64748b; text-align: left; text-transform: uppercase; letter-spacing: .5px; }
    .sites-table td { padding: 14px 16px; border-top: 1px solid #f1f5f9; font-size: 0.875rem; }
...
export class DashboardComponent implements OnInit, AfterViewInit {
  ...
  get globalTotal() { return this.sites.reduce((s, x) => s + x.totalCo2Kg, 0); }
  get avgCo2PerM2() { return this.sites.length ? this.sites.reduce((s, x) => s + x.co2PerM2, 0) / this.sites.length : 0; }
  get avgCo2PerEmployee() { return this.sites.length ? this.sites.reduce((s, x) => s + x.co2PerEmployee, 0) / this.sites.length : 0; }
