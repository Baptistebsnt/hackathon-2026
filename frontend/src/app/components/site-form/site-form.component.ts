...
@Component({
        ...
        <div class="kpis">
          ...
          <div class="kpi operation">
            <span class="kpi-value"
              >{{ result.operationCo2KgPerYear / 1000 | number: '1.1-1' }} t/an</span
            >
  ...
  styles: [
    `
      ...
      .kpi.construction .kpi-value {
        color: #d97706;
      }
      .kpi.operation .kpi-value {
        color: #2563eb;
      }
      .error {
        color: #dc2626;
...
export class SiteFormComponent {
  ...
  onSubmit(): void {
    ...
    this.siteService.createSite(this.form.value).subscribe({
      ...
      error: (_err) => {
        this.error = 'Erreur lors du calcul. Vérifiez que le backend est démarré.';
        this.loading = false;
      },
    });
  }
